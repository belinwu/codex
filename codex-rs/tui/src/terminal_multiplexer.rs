...
const TMUX_FORK_PLACEMENTS: &[ForkPanePlacement] = &[
    ForkPanePlacement::Up,
    ForkPanePlacement::Down,
    ForkPanePlacement::Left,
    ForkPanePlacement::Right,
];
const ZELLIJ_FORK_PLACEMENTS: &[ForkPanePlacement] = &[
    ForkPanePlacement::Right,
    ForkPanePlacement::Down,
...
fn fork_spawn_config(
    ...
    match multiplexer {
        Multiplexer::Zellij {} => MultiplexerSpawnConfig {
            program: "zellij",
            args: build_zellij_new_pane_args(&command, placement),
...
fn supported_fork_placements(multiplexer: Option<&Multiplexer>) -> &'static [ForkPanePlacement] {
    match multiplexer {
        Some(Multiplexer::Zellij {}) => ZELLIJ_FORK_PLACEMENTS,
        Some(Multiplexer::Tmux { .. }) => TMUX_FORK_PLACEMENTS,
        None => &[],
    }
}

pub(crate) fn fork_command_usage(multiplexer: Option<&Multiplexer>) -> String {
    let placements = supported_fork_placements(multiplexer);
    ...
    let options = placements
        ...
        .map(|placement| match placement {
            ForkPanePlacement::Up => "up",
            ForkPanePlacement::Down => "down",
            ForkPanePlacement::Left => "left",
            ForkPanePlacement::Right => "right",
            ForkPanePlacement::Float => "float",
        })
        .collect::<Vec<_>>()
        .join("|");
...
fn validate_fork_placement_for_multiplexer(
    ...
    match multiplexer {
        Multiplexer::Zellij {} => {
            if placement.is_none_or(|placement| ZELLIJ_FORK_PLACEMENTS.contains(&placement)) {
                Ok(())
            } else {
        ...
        Multiplexer::Tmux { .. } => {
            if placement.is_none_or(|placement| TMUX_FORK_PLACEMENTS.contains(&placement)) {
                Ok(())
            } else {
...
pub(crate) async fn spawn_fork_in_new_pane(
    ...
    .await
    {
        ...
        Ok(Err(err)) => ForkPaneSpawnResult::Failed(format!("failed to run {program}: {err}")),
        Err(err) => ForkPaneSpawnResult::Failed(format!("failed to spawn {program} pane: {err}")),
    }
}
